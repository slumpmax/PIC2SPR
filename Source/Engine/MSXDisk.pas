...
uses
  Math, Classes, DateUtils, SysUtils;
...
type
  TMSXDiskResult = (mdrDone = 0, mdrDiskFull, mdrDirectoryFull,
    mdrDirectoryExists, mdrDirectoryNotEmpty, mdrFileNotFound);
  TMSXDiskSortType = (mstNone = -1, mstNameExt, mstExtName, mstSize,
    mstDateTime);
  ...
  public
    ...
    property ModifyDateTime: TDateTime read GetModifyDateTime
      write SetModifyDateTime;
    // constructor Create(NewEntry: Boolean);
    procedure Clear(NewEntry: Boolean = False);
    function NameText: string;
    function ExtText: string;
  ...
  public
    ...
    property Dirs[AIndex: Integer]: PDirEntry read GetDirs; default;
  end;
  ...
  public
    ...
    OemName: array [0 .. 7] of AnsiChar; // $03
    ...
    BootCode: array [0 .. 483] of Byte;
    // $1C..$1FF, Or information to replace disk
  ...
  public
    ...
    function AddFromStream(AStream: TStream; AFileName: string; ASize: Integer): Integer;
    ...
    procedure SetCurrentDirList(AList: TDirEntryList);
    ...
    property FatTable[ACluster: Integer]: Integer read GetFatTable
      write SetFatTable;
    property DirTable[AIndex: Integer]: TDirEntry read GetDirTable
      write SetDirTable;
  ...
  TMSXDiskFileStream = class(TMemoryStream)
  private
    FFileName: string;
    FDisk: TMSXDisk;
    FMode: Word;
  public
    constructor Create(AFileName: string; ADisk: TMSXDisk; AMode: Word);
    destructor Destroy; override;
...
begin
  ...
  begin
    if not pdir2.IsDirectory then
      Result := -1
  end
  else if pdir2.IsDirectory then
    Result := 1;
  if Result = 0 then
  begin
    Result := CompareText(pdir1.NameText, pdir2.NameText);
    if Result = 0 then
      Result := CompareText(pdir1.ExtText, pdir2.ExtText);
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if Result = 0 then
        Result := pdir1.ModifyTime.Value - pdir2.ModifyTime.Value;
    end;
    if Result = 0 then
      Result := CompareText(pdir1.NameText, pdir2.NameText);
    if Result = 0 then
...
begin
  ...
  begin
    if pdir1.IsDirectory then
      ...
      Result := CompareText(pdir2.NameText, pdir1.NameText);
    if Result = 0 then
      Result := CompareText(pdir1.ExtText, pdir2.ExtText);
  end;
end;
...
begin
  ...
  begin
    if pdir1.IsDirectory then
    begin
      Result := CompareText(pdir1.NameText, pdir1.NameText);
      if Result = 0 then
        Result := CompareText(pdir1.NameText, pdir2.NameText);
    end
    else
    begin
      Result := CompareText(pdir2.ExtText, pdir1.ExtText);
      if Result = 0 then
        Result := CompareText(pdir1.NameText, pdir2.NameText);
    end;
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if Result = 0 then
        Result := pdir2.ModifyTime.Value - pdir1.ModifyTime.Value;
    end;
    if Result = 0 then
      Result := CompareText(pdir1.NameText, pdir2.NameText);
    if Result = 0 then
...
begin
  if FDescOrdered then
    case FSortType of
      mstNameExt:
        inherited Sort(DescCompareNameExt);
      mstExtName:
        inherited Sort(DescCompareExtName);
      mstSize:
        inherited Sort(DescCompareSize);
      mstDateTime:
        inherited Sort(DescCompareDateTime);
    end
  else
    case FSortType of
      mstNameExt:
        inherited Sort(CompareNameExt);
      mstExtName:
        inherited Sort(CompareExtName);
      mstSize:
        inherited Sort(CompareSize);
      mstDateTime:
        inherited Sort(CompareDateTime);
    end;
end;
...
begin
  Result := TrimRight(ExtText);
  if Result <> '' then
    Result := '.' + Result;
  Result := TrimRight(NameText) + Result;
end;
...
begin
  ...
  FillChar(Ext, 3, ' ');
  s := AnsiString(AName);
  Move(s[1], Name, Min(Length(s), 8));
  s := AnsiString(se);
  Move(s[1], Ext, Min(Length(s), 3));
end;
...
function TDirEntry.NameText: string;
begin
  Result := string(Name);
end;
...
function TDirEntry.ExtText: string;
begin
  Result := string(Ext);
end;
...
begin
  ...
  begin
    ...
    repeat
      Inc(n);
      dir := DirTable[n];
      if (dir.FileName <> '.') and (dir.FileName <> '..') and
        not dir.Deleted and dir.Existed then
        Inc(Result);
...
var
  ndir, nfat, fsize, coffset: Integer;
...
const
  MSXBootCode: array [0 .. 243] of Byte = ($00, $00, $D0, $ED, $53, $58, $C0,
    $32, $C2, $C0, $36, $55, $23, $36, $C0, $31, $1F, $F5, $11, $9D, $C0, $0E,
    $0F, $CD, $7D, $F3, $3C, $28, $28, $11, $00, $01, $0E, $1A, $CD, $7D, $F3,
    $21, $01, $00, $22, $AB, $C0, $21, $00, $3F, $11, $9D, $C0, $0E, $27, $CD,
    $7D, $F3, $C3, $00, $01, $57, $C0, $CD, $00, $00, $79, $E6, $FE, $FE, $02,
    $20, $07, $3A, $C2, $C0, $A7, $CA, $22, $40, $11, $77, $C0, $0E, $09, $CD,
    $7D, $F3, $0E, $07, $CD, $7D, $F3, $18, $B4, $42, $6F, $6F, $74, $20, $65,
    $72, $72, $6F, $72, $0D, $0A, $50, $72, $65, $73, $73, $20, $61, $6E, $79,
    $20, $6B, $65, $79, $20, $66, $6F, $72, $20, $72, $65, $74, $72, $79, $0D,
    $0A, $24, $00, $4D, $53, $58, $44, $4F, $53, $20, $20, $53, $59, $53, $00,
    $00, $00, $00, $00, $00, $00, $00, $00, $00, $00, $00, $00, $00, $00, $00,
    $00, $00, $00, $00, $00, $00, $00, $00, $00, $00, $00, $00, $00, $00, $00,
    $00, $00, $00, $00, $00, $00, $F3, $2A, $51, $F3, $11, $00, $01, $19, $01,
    $00, $01, $11, $00, $C1, $ED, $B0, $3A, $EE, $C0, $47, $11, $EF, $C0, $21,
    $00, $00, $CD, $51, $52, $F3, $76, $C9, $18, $64, $3A, $AF, $80, $F9, $CA,
    $6D, $48, $D3, $A5, $0C, $8C, $2F, $9C, $CB, $E9, $89, $D2, $00, $32, $26,
    $40, $94, $61, $19, $20, $E6, $80, $6D, $8A, $00, $00, $00);
  MSXBootCode2: array [0 .. 163] of Byte = ($00, $00, $18, $10, $56, $4F, $4C,
    $5F, $49, $44, $00, $1A, $11, $04, $5D, $00, $00, $00, $00, $00, $D0, $ED,
    $53, $6A, $C0, $32, $72, $C0, $36, $67, $23, $36, $C0, $31, $1F, $F5, $11,
    $AB, $C0, $0E, $0F, $CD, $7D, $F3, $3C, $28, $26, $11, $00, $01, $0E, $1A,
    $CD, $7D, $F3, $21, $01, $00, $22, $B9, $C0, $21, $00, $3F, $11, $AB, $C0,
    $0E, $27, $CD, $7D, $F3, $C3, $00, $01, $69, $C0, $CD, $00, $00, $79, $E6,
    $FE, $D6, $02, $F6, $00, $CA, $22, $40, $11, $85, $C0, $0E, $09, $CD, $7D,
    $F3, $0E, $07, $CD, $7D, $F3, $18, $B8, $42, $6F, $6F, $74, $20, $65, $72,
    $72, $6F, $72, $0D, $0A, $50, $72, $65, $73, $73, $20, $61, $6E, $79, $20,
    $6B, $65, $79, $20, $66, $6F, $72, $20, $72, $65, $74, $72, $79, $0D, $0A,
    $24, $00, $4D, $53, $58, $44, $4F, $53, $20, $20, $53, $59, $53, $00, $00,
...
begin
  ...
  begin
    ...
    if (ACluster and 1) > 0 then
      ...
      Result := Buffer[p] or ((Buffer[p + 1] and 15) shl 8);
  end;
end;
...
begin
  ...
  begin
    ...
    if FFatTableCount > 1 then
      Move(Buffer[FFatOffset], Buffer[FFatOffset + FFatSize], FFatSize);
  end;
end;

procedure TMSXDisk.SetCurrentDirList(AList: TDirEntryList);
var
  dir: TDirEntry;
  ndir: Integer;
begin
  dir.Clear;
  ndir := 0;
  while (ndir < AList.Count) and (ndir < FDirCount) do
  begin
    if ndir < AList.Count then
      DirTable[ndir] := AList[ndir]^
    else DirTable[ndir] := dir;
    Inc(ndir);
  end;
end;
...
begin
  ...
  repeat
    dir := DirTable[n];
    if dir.Existed then
      if not dir.Deleted and ((dir.Attr and (mfaDirectory or mfaLabel)) = 0)
      then
        Inc(Result);
    Inc(n);
...
begin
  ...
  try
    Result := AddFromStream(fs, AFileName, fs.Size);
  finally
    fs.Free;
  end;
end;
...
// Result = number of file(s) to add (0 or 1)
// LastError = error result
function TMSXDisk.AddFromStream(AStream: TStream; AFileName: string; ASize: Integer): Integer;
var
  ndir, nfat, n, coffset: Integer;
  dir, subdir: TDirEntry;
begin
  Result := 0;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        while (n < FClusterCount) and (FatTable[n] <> $FFF) and
          (FatTable[n] <> 0) do
        begin
          n := FatTable[n];
  ...
  begin
    ...
    dir.Cluster := nfat;
    // DirTable[ndir] := dir;
    while (ASize > 0) and (nfat >= 0) do
    begin
      n := Min(ASize, FClusterSize);
      coffset := GetClusterOffset(nfat);
      AStream.Read(Buffer[coffset], n);
...
begin
  ...
  begin
    ...
    FDataOffset := FDirOffset + ((FDirCount * 32 + FSectorSize - 1)
      div FSectorSize) * FSectorSize;
    FClusterCount := ((FSectorCount * FSectorSize) - FDataOffset)
      div FClusterSize + 2;
...
begin
  ...
  begin
    ...
    begin
      ...
      else if dir.FileName <> '.' then
        FDirectory := IncludePathDelim(FDirectory) + dir.FileName +
          MSXPathDelim;
    end;
  end;
end;
...
begin
  n := Length(ADirName);
  if (n > 0) and (ADirName[n] = MSXPathDelim) then
    Dec(n);
...
begin
  ...
  while dir.Existed and (Result < 0) do
  begin
    if (dir.FileName = ADirName) and ((dir.Attr and mfaDirectory) <> 0) then
      Result := n;
    Inc(n);
    dir := DirTable[n];
  end;
...
begin
  ...
  if (FBootRecord.JumpCode[0] = $EB) and
    ((FBootRecord.MediaType = $F8) or (FBootRecord.MediaType = $F9)) then
  begin
    Changed;
    if AFileName <> '' then
      FFileName := AFileName;
...
constructor TMSXDiskFileStream.Create(AFileName: string; ADisk: TMSXDisk; AMode: Word);
begin
  inherited Create;
  FFileName := AFileName;
  FDisk := ADisk;
  ...
  case FMode of
    ...
    begin
      if not FDisk.FileNameExists(FFileName) then
        raise Exception.Create('MSXDisk file not found: ' + FFileName);
      case FMode of
        fmOpenRead, fmOpenReadWrite:
        begin
          FDisk.ExtractToStream(Self, FFileName);
          Seek(0, soBeginning);
        end;
      end;
    end;
  else
    raise Exception.Create('TMSXDiskFileStream create mode not supported.');
  end;
end;

destructor TMSXDiskFileStream.Destroy;
begin
  case FMode of
    fmOpenWrite, fmOpenReadWrite, fmCreate:
    begin
      Seek(0, soBeginning);
      FDisk.AddFromStream(Self, FFileName, Size);
    end;
  end;
  inherited Destroy;
end;

function TMSXDiskFileStream.Read(var Buffer; Count: Longint): Longint;
begin
  case FMode of
    fmOpenRead, fmOpenReadWrite: Result := inherited Read(Buffer, Count);
  else
    raise Exception.Create('TMSXDiskFileStream open in write only mode.');
  end;
end;

function TMSXDiskFileStream.Write(const Buffer; Count: Longint): Longint;
begin
  if FMode = fmOpenRead then
    raise Exception.Create('TMSXDiskFileStream open in write only mode.')
  else Result := inherited Write(Buffer, Count);
end;
