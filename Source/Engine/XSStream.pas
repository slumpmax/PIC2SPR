...
type
  ...
  private
    ...
    winpos, freepos, replace_cnt, strlen, strpos: Word;
    // * first free pos in moving window */

    procedure strout(str: PBytes; strlen: Byte; strpos: Word);
    // * send a string to the output           */
    procedure charout(ch: Byte); // * send a char to the output             */
    procedure cnvtohuffcodes(nodeptr: PHufNodes; bitcode: Word;
      nrbits: SmallInt); // * zet de boom om in huffman codes  */
    function findmsb(n: Byte): Byte;
    // * vind hoogste bit van een getal        */
    function bitglen: Cardinal; // * get length of a bitstream file*/
    procedure bitspos(newpos: Cardinal);
    // * go to a position in a bitstrem file   */
    ...
    procedure DeleteString(strpos: Word);
    // * delete a string from the indexptrs */
    procedure fAddString(winpos: Word); // * start pos of the string to add */
    function AddString(winpos: Word; var strposptr: Word): Word;
    // * add str with srch for best match      */

    procedure mkheader(name: string);
    // * maak de header                        */
    procedure savelens(orglen, destlen: Cardinal);
    // * save the org len and dest len         */
  public
  ...
  public
    ...
    constructor Create(const AStreamToEncode: TStream;
      AOriginalName: string); overload;
    destructor Destroy; override;
...
{$IFDEF bits5}
  cpdext: array [0 .. 31] of Word = ( // Extra bits for distance codes
    0, 0, 0, 0, 1, 1, 2, 2, 2, 2, 3, 3, 3, 4, 4, 4, 5, 5, 5, 5, 5, 6, 6, 6, 6,
    7, 7, 8, 10, 11, 11, 11);
{$ELSE}
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        else if (tempw < l2pos[0].weight) then
          l2pos := hufpos;
      end;
      Inc(hufpos);
      tempw := hufpos[0].weight;
...
begin
  ...
  FOrgName := '';
  buf := #0#0#0#0;
  if FStream.Read(buf[1], 4) <> 4 then
    Exit;

  if buf <> 'PCK'#8 then
    Exit;

  if FStream.Read(FOrgSize, 4) <> 4 then
    Exit;
  if FStream.Read(FPackedSize, 4) <> 4 then
    Exit;
...
begin
  ...
  if chkheader = -1 then
    raise Exception.Create('Invalid header in XSA file.');
  CodeLength := 0;
  FEOF := False;
end;
...
begin
  Result := 0;
  if (readbufcnt >= outbufcnt - slwinsize) and (CodeLength <> maxstrlen + 1)
  then
  begin
...
begin
  ...
  begin
    ...
    begin
      ...
      Count := 0;
      while strpos >= cpdist[Count + 1] do
        Inc(Count);
      Inc(tblsizes[Count]);
    ...
    begin
      mkhuftbl; // make the huffman table */
      cnvtohuffcodes(@huftbl[2 * tblsize - 2], 0, 0);
      // conv into bitstream info */
    end
    else
      Dec(updhufcnt);
  end;
...
// ****************************************************************/
procedure TXSEncode.cnvtohuffcodes(nodeptr: PHufNodes; bitcode: Word;
  nrbits: SmallInt);
var
  node: Word;
begin
...
begin
  mskflg := 128;
  while ((n and mskflg) = 0) and (mskflg <> 0) do
    mskflg := mskflg shr 1;
  Result := mskflg;
end;
...
// ****************************************************************/
procedure TXSEncode.fAddString(winpos: Word);
          ...
          begin
            ...
            begin
              if strlen > lookahead_cnt then
                strlen := lookahead_cnt; // strlen must be <= lookahead_cnt */
              replace_cnt := strlen;
              strout(@mwindow[strpos], replace_cnt, (winpos - strpos) and
                slwinmask);
            end;
          end;
...
begin
  ...
  for temp := 1 to Length(name) do
    fileout(Ord(name[temp]));
  fileout(0);
end;
...
constructor TXSStream.Create(const AStreamToEncode: TStream;
...
begin
  if FWrited then
    ...
    Result := FDecoder.FOrgName;
end;

function TXSStream.GetSize: Int64;
begin
  if FWrited then
    raise Exception.Create('Can not get size on XSA encoder mode.');
...
begin
  if FWrited then
    raise Exception.Create('Can not read data on XSA encoder mode.');
  Result := 0;
  bufpos := @Buffer;
...
begin
  if FWrited then
    raise Exception.Create('Can not seek offset on XSA encoder mode.');
  case Origin of
    soEnd:
      n := FDecoder.OrgSize - Offset;
    soBeginning:
      n := Offset - FDecoder.nrwrite;
  else
    n := Offset;
  end;
...
begin
  if not FWrited then
    raise Exception.Create('Can not write data on XSA decoder mode.');
  Result := 0;
  bufpos := @Buffer;
